...
function f_init() {
    ...
    frm_pay.EP_order_no.value = `ORDER_${dayjs().format('YYYYMMDDHHmmssSSS')}`;   //가맹점 주문번호    
    //결제수단(select)
    frm_pay.EP_currency.value = "00";                                    //통화코드 : 00-원
    frm_pay.EP_product_nm.value = "Managed Cloud Service";                            //상품명
    ...
    frm_pay.EP_vacct_end_date.value = dayjs().add(7, 'day').format('YYYYMMDD');    //입금 만료 날짜
    frm_pay.EP_vacct_end_time.value = dayjs().add(7, 'day').format('HHMMss');                                //입금 만료 시간
}
