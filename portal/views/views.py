...
def search_orgs(request: HttpRequest) -> HttpResponse:
    ...
    if query is None or query == "":
        result = result.filter(is_active=True).values("name", "slug", "id", "orgkey__key")[:8]
    else:
        result = result.filter(name__contains=query, is_active=True).values("name", "slug", "id", "orgkey__key")
    print(list(result))
